<template>
  <div class="w-full h-full">
    <div v-if="loading" class="flex items-center justify-center h-full">
      <div class="text-gray-500">Loading fuel data...</div>
    </div>
    <v-chart 
 ...
 * @description UI Component for FuelConsumptionChart.
 *
...
async function fetchActualData() {
  ...
  try {
    ...
    // Use the same method as the fuel meter pages
    ws.send(JSON.stringify({
      ...
      params: {
        ...
        payload: {
          ...
          params: {
            channels: ['fuelmeter0/Consumption'],
    ...
    console.error('Error fetching actual fuel data:', error)
    return null
  }
}
...
function generateForecastData(actualData) {
  ...
  const actualValues = actualData.data['fuelmeter0/Consumption'] || []
  ...
  return {
    ...
    data: {
      'fuelmeter0/Consumption': forecastValues
    }
  }
}
...
async function fetchData() {
  ...
  try {
    ...
    if (actualData && actualData.timestamps && actualData.data) {
      ...
      console.warn('No actual fuel data received')
    }
  } catch (error) {
...
function processChartData(actualData, forecastData) {
  let actualValues = actualData.data['fuelmeter0/Consumption'] || []
  let forecastValues = forecastData ? forecastData.data['fuelmeter0/Consumption'] || [] : []
...
function calculateAndEmitStats(actualData, forecastData) {
  ...
  emit('update-stats', {
    actualAvg: (actualAvg / 1000).toFixed(2), // Convert milliliters to kg
    forecastAvg: (forecastAvg / 1000).toFixed(2), // Convert milliliters to kg
    ...
    series: [
      {
        name: 'Actual',
        type: 'bar',
        data: chartData.value.actualData.map(val => val / 1000), // Convert milliliters to kg
        itemStyle: {
          color: '#ef4444'
        },
        barWidth: '40%'
      },
      {
        name: 'Forecast',
        type: 'bar',
        data: chartData.value.forecastData.map(val => val / 1000), // Convert milliliters to kg
        itemStyle: {
          color: '#f59e0b'
        },
        barWidth: '40%'
