    ...
    <div class="grid grid-cols-1 gap-4 w-full">
      ...
      <div class="bg-gray-800 bg-opacity-30 rounded-lg p-4 border border-gray-700">
        <div class="flex items-center justify-between">
          ...
          <div class="text-right">
            <div class="text-lg font-bold text-green-500">{{ pressure.toFixed(1) }} bar</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-gray-800 bg-opacity-30 rounded-lg p-4 border border-gray-700">
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            <i class="bi bi-droplet text-blue-500 text-lg mr-2"></i>
            <span class="text-sm text-gray-400">Flow Status</span>
          </div>
          <div class="text-right">
            <div 
              class="text-lg font-bold"
              :class="flow > 7.0 ? 'text-green-500' : 'text-yellow-500'"
            >
              {{ flow > 7.0 ? 'ACTIVE' : 'LOW' }}
            </div>
          </div>
...
<script setup>
/**
 * @component FuelFlowVisualization
 * @description UI Component for FuelFlowVisualization.
 *
