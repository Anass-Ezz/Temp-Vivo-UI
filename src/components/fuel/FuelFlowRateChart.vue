...
<script setup>
/**
 * @component FuelFlowRateChart
 * @description UI Component for FuelFlowRateChart.
 *
...
// Fuel flow rate formatting function
...
async function fetchData() {
  ...
  try {
    ...
    // Use queryHistoricTimeseriesData for fuel flow rate data
    ...
    console.error('Error fetching fuel flow rate data:', error)
    loading.value = false
  }
}
...
function processChartData(result) {
  ...
  const seriesData = props.channels.map(channelId => {
    ...
    // Apply flow rate scaling: g/s ÷ 1000 = kg/s
    ...
    tooltip: {
      ...
      formatter: (params) => {
        ...
        params.forEach(param => {
          const formattedValue = formatFuelFlowWithUnit(param.value)
