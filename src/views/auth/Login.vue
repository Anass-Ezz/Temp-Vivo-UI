...
const login = async () => {
    ...
    if (ws.readyState !== WebSocket.OPEN) {
        ...
        track('auth.login.failure', {
            ...
            requiresToken: false,
            metadata: { reason: 'connection_not_ready' }
        });
        flush();
        toast.add({
            severity: 'error',
    ...
    const handleMessage = (event) => {
        ...
        if (msg.id === id) {
            ...
            if (msg.result) {
                ...
                const user = msg.result.user || null;
                // Always use localStorage so the session persists across tabs.
                ...
                track('auth.login.success', {
                    ...
                    service: 'auth',
                    result: 'success',
                    userEmail: user?.email || user?.login || user?.id || email.value,
                    userId: user?.id || user?.uid || email.value,
                    userRole: user?.globalRole || user?.role || 'user'
                });
                flush();
